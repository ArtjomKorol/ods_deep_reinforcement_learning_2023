    ...
    def fit(self, elite_trajectories):
        ...
        for state in range(self.state_n):
            if np.sum(new_model[state]) > 0:
                new_model[state] += self.smooth_lambda
...
total_mean_rewards = []
param_range = [0.01, 0.05, 0.1, 0.5, 1, 1.5]
...
plt.title(f"Rewards dependence on labmda, q_param: {q_param}, trajectory_len: {trajectory_n}, max_len: {max_len}", fontsize = 16)
plt.legend(loc = "best")
plt.savefig('lambda_rewards')
