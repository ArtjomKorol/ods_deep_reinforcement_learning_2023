...
total_mean_rewards = []
for q_param in np.arange(0.8, 1, 0.1):
...
for rewards, trajectory_len in zip(total_mean_rewards, np.arange(0.8, 1, 0.1)):
    plt.plot(np.arange(0, iteration_n), rewards, label = np.round(trajectory_len, 2))
    
plt.xlabel("Iteration", fontsize = 14)
plt.ylabel("Mean total reward", fontsize = 14)
plt.title(f"Rewards dependence on q_param, trajectory_len: {trajectory_n}, max_len: {max_len}", fontsize = 16)
plt.legend(loc = "best")
plt.savefig('reward_q_param_4000_trajectories')
